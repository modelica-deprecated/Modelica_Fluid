...
package StatePorts_A 
  "Implementation A with dynamic states on the connector effort variables (similar to the ThermoFluid library)" 
...
The component volume <tt>V_lumped</tt> is also a variable which needs to be set in the extending class to complete the model.
  ...
  equation 
    ...
    // (independent of current mass flow direction, because non-alternating 
    // connections such as FM-FM are illegal)
  ...
  equation 
    assert(false, "A PartialTransportIsenthalpic with PortA interfaces is illegal in the StatePorts_A approach.");
  ...
  redeclare replaceable partial model extends PartialTransportIsenthalpicAB 
    "Partial isenthalpic element transporting fluid between two ports without storing mass or energy (a Port_a and Port_b each, illegal in this approach)" 
    
  equation 
    assert(false, "A PartialTransportIsenthalpic with a PortA and PortB is illegal in the StatePorts_A approach.");
  ...
  redeclare replaceable partial model extends PartialTransportIsentropicAA 
    "Partial isentropic element transporting fluid between two ports without storing mass or energy (two Port_a's, illegal in this approach)" 
    
  equation 
    assert(false, "A PartialTransportIsentropic with PortA interfaces is illegal in the StatePorts_A approach.");
  ...
  redeclare replaceable partial model extends PartialTransportIsentropicAB 
    "Partial isentropic element transporting fluid between two ports without storing mass or energy (a Port_a and Port_b each, illegal in this approach)" 
    
  equation 
    assert(false, "A PartialTransportIsentropic with a PortA and PortB is illegal in the StatePorts_A approach.");
  ...
  equation 
    assert(false, "In the StatePorts_A approach, only dynamic junctions can be used.");
  ...
  equation 
    port.p = medium.p;
    port.H_flow = port.m_flow*noEvent(if port.m_flow > 0 then port.h else 
      medium.h);
    port.mXi_flow = port.m_flow*noEvent(if port.m_flow > 0 then port.Xi else 
            medium.Xi);
  ...
  equation 
    ...
    // Specific enthalpy
    port_a.h = medium[1].h;
    port_b.H_flow = noEvent(port_b.m_flow*(if port_b.m_flow > 0 then port_b.h else 
            medium[n].h));
    // Substance mass fractions
    port_a.Xi = medium[1].Xi;
    port_b.mXi_flow = noEvent(port_b.m_flow*(if port_b.m_flow > 0 then 
      port_b.Xi else medium[n].Xi));
...
<li><i>04 Mar 2006</i>
  ...
  equation 
    ...
    // Connector equations
    // Specific enthalpy
    port_a.H_flow = noEvent(port_a.m_flow*(if port_a.m_flow > 0 then port_a.h else 
            medium[1].h));
    port_b.H_flow = noEvent(port_b.m_flow*(if port_b.m_flow > 0 then port_b.h else 
            medium[n].h));
    // Substance mass fractions
    port_a.Xi = noEvent(port_a.m_flow*(if port_a.m_flow > 0 then port_a.Xi else 
            medium[1].Xi));
    port_b.mXi_flow = noEvent(port_b.m_flow*(if port_b.m_flow > 0 then 
      port_b.Xi else medium[n].Xi));
