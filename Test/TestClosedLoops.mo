...
package TestClosedLoops
  model HeatingSystemTankCompressible "Simple model of a heating system"
    ...
    Modelica.Blocks.Interfaces.RealOutput tankLevel 
                                   annotation (Placement(transformation(extent={{-56,50},
              {-44,62}},          rotation=0)));
    ...
    Modelica.Thermal.HeatTransfer.Sources.FixedTemperature ambientTemperature1(
                                                                      T=system.T_ambient) 
      annotation (Placement(transformation(extent={{-95,49},{-85,59}}, rotation=
             0)));
    Sources.LoopBreaker loopBreaker(redeclare package Medium = Medium, p=111000) 
...
"),   experiment(StopTime=6000),
    ...
    connect(loopBreaker.port, pump.port_a) annotation (Line(
        points={{-50,28.2},{-50,20}},
        color={0,127,255},
        smooth=Smooth.None));
  end HeatingSystemTankCompressible;

  model HeatingSystemTankCompressibleStaticMassEnergy
    extends HeatingSystemTankCompressible(system(massDynamics=Modelica_Fluid.Types.Dynamics.SteadyState,
          energyDynamics=Modelica_Fluid.Types.Dynamics.SteadyState));
    annotation(experiment(StopTime = 6000));
  end HeatingSystemTankCompressibleStaticMassEnergy;

  model HeatingSystemTankIncompressible
    extends HeatingSystemTankCompressible(
      redeclare package Medium = 
          Modelica.Media.Water.ConstantPropertyLiquidWater, system(
          energyDynamics=Modelica_Fluid.Types.Dynamics.SteadyState));
    annotation(experiment(StopTime = 6000));
  end HeatingSystemTankIncompressible;

  model HeatingSystemNoTankCompressible "Simple model of a heating system"
     replaceable package Medium = 
        Modelica.Media.Water.StandardWater 
       constrainedby Modelica.Media.Interfaces.PartialMedium;
    ...
    Sources.LoopBreaker loopBreaker(redeclare package Medium = Medium, p=111100) 
      annotation (Placement(transformation(extent={{-60,28},{-40,48}})));
  equation
    connect(massFlowRate.m_flow, flowRate)        annotation (Line(points={{-10,31},
            {-10,40},{0,40}},                     color={0,0,127}));
...
"),   experiment(StopTime=6000),
      Commands(file(ensureSimulated=true)=
          "Scripts/Examples/HeatingSystem/plotResults.mos" "plotResults"));
    connect(radiator.port_b, pump.port_a) annotation (Line(
        points={{0,-70},{-74,-70},{-74,20},{-50,20}},
        color={0,127,255},
        smooth=Smooth.None));
    connect(loopBreaker.port, pump.port_a) annotation (Line(
        points={{-50,28.2},{-50,20}},
        color={0,127,255},
        smooth=Smooth.None));
  end HeatingSystemNoTankCompressible;

  model HeatingSystemNoTankCompressibleStaticMassEnergy
    extends HeatingSystemNoTankCompressible(system(massDynamics=Modelica_Fluid.Types.Dynamics.SteadyState,
          energyDynamics=Modelica_Fluid.Types.Dynamics.SteadyState));
    annotation(experiment(StopTime = 6000));
  end HeatingSystemNoTankCompressibleStaticMassEnergy;

  model HeatingSystemNoTankIncompressible
    extends HeatingSystemNoTankCompressible(
      redeclare package Medium = 
          Modelica.Media.Water.ConstantPropertyLiquidWater);
    annotation(experiment(StopTime = 6000));
  end HeatingSystemNoTankIncompressible;
end TestClosedLoops;
